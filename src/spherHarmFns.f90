...
contains
    subroutine genLegendreFns ( nPts, theta_, m, nBFns, &
        ...
        real(kind=DBL), dimension ( nTh ) :: theta
        real(kind=DBL), dimension ( nTh, nTh ) :: coeffMatrix
        real(kind=DBL) :: dTh, term0, term1, term2, term3, term4
        real(kind=DBL), dimension ( nTh ) :: kVals

        complex(kind=DBL), dimension ( nTh ) :: eigenValuesComplex
        complex(KIND=DBL), dimension ( nTh, nTh ) :: eigenVectorsComplex
        real(kind=DBL), dimension ( nTh ) :: eigenValues, nValsTmp, nVals
        integer, dimension ( nTh ) :: iiNVals
        real(kind=DBL), dimension ( nTh, nTh ) :: eigenVectors
        real(kind=DBL), allocatable, dimension ( :, : ) :: eigenVectors_, dEigenVectors_
        ...
        integer :: iiKeep ( maxK ), aStatus!, iiGoodK ( maxK )
...
!           ldVr, Work, lWork, info )
        ...
        call evcrg ( transpose ( coeffMatrix ), eigenValuesComplex, eigenVectorsComplex )
